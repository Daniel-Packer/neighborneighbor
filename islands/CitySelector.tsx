...
export const AVAILABLE_CITIES: CityLocation[] = [
  { name: "Seattle, WA", coordinates: [47.6062, -122.3321], zoom: 12, key: "seattle" },
  { name: "Portland, OR", coordinates: [45.5152, -122.6784], zoom: 12, key: "portland" },
  { name: "New York, NY", coordinates: [40.7128, -74.0060], zoom: 12, key: "newyork" },
  { name: "Boston, MA", coordinates: [42.3601, -71.0589], zoom: 12, key: "boston" },
  { name: "San Francisco, CA", coordinates: [37.7749, -122.4194], zoom: 12, key: "sanfrancisco" },
  { name: "Los Angeles, CA", coordinates: [34.0522, -118.2437], zoom: 11, key: "losangeles" },
  { name: "Chicago, IL", coordinates: [41.8781, -87.6298], zoom: 11, key: "chicago" },
  { name: "Detroit, MI", coordinates: [42.3314, -83.0458], zoom: 12, key: "detroit" },
  { name: "Austin, TX", coordinates: [30.2672, -97.7431], zoom: 12, key: "austin" },
  { name: "Houston, TX", coordinates: [29.7604, -95.3698], zoom: 11, key: "houston" },
  { name: "Miami, FL", coordinates: [25.7617, -80.1918], zoom: 12, key: "miami" },
  { name: "Denver, CO", coordinates: [39.7392, -104.9903], zoom: 12, key: "denver" }
];

interface CitySelectorProps {
  onCitySelectionChange: (leftCity: CityLocation, rightCity: CityLocation) => void;
...
export default function CitySelector({ 
  ...
  const getSelectedCities = (): [CityLocation, CityLocation] => {
    const left = AVAILABLE_CITIES.find(city => city.key === leftCityKey.value) || AVAILABLE_CITIES[0];
    const right = AVAILABLE_CITIES.find(city => city.key === rightCityKey.value) || AVAILABLE_CITIES[1];
    return [left, right];
  };
  ...
  return (
    <div class="flex flex-wrap gap-3">
      <div class="flex items-center">
        <label htmlFor="leftCitySelect" class="mr-2 text-sm font-medium">Left map:</label>
        <select 
          id="leftCitySelect"
      ...
      <div class="flex items-center">
        <label htmlFor="rightCitySelect" class="mr-2 text-sm font-medium">Right map:</label>
        <select 
          id="rightCitySelect"
          class="px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
